...
function changePosition() {
    ...
    let newX = maxLeft * Math.random();//785
    let newY = maxTop * Math.random();//26
    div.style.left = newX + 'px';
    div.style.top = newY + 'px';
    div.style.width = div.clientWidth + 10 + 'px'
    if(div.clientWidth >= (wrapper.clientWidth-50) || div.clientHeight>=(wrapper.clientHeight-50)){count++; div.style.width = '50px'; countP.innerHTML = count; img.src = imgArr[i]; i++; if(i >= imgArr){i = 0;}}
    console.log(newX);
    console.log(newY);
